...
public class WifiService extends Service {
    ...
    private ArrayList<AccessPoint_>      mAPList = new ArrayList<AccessPoint_>();
    ...
    public ArrayList<AccessPoint_> getAPList() {
        return mAPList;
    }

    public interface OnUpdateAccessPointListener {
        abstract void onUpdateAPListener(ArrayList<AccessPoint_> apns);
...
//                setOffMessage();
    ...
    private void updateConnectionState(NetworkInfo.DetailedState state) {
        ...
        for (AccessPoint_ accessPoint : mAPList) {
    ...
    private ArrayList<AccessPoint_> constructAccessPoints() {
        ArrayList<AccessPoint_> accessPoints = new ArrayList<AccessPoint_>();
        /** Lookup table to more quickly update AccessPoints by only considering objects with the
         * correct SSID.  Maps SSID -> List of AccessPoints with the given SSID.  */
        Multimap<String, AccessPoint_> apMap = new Multimap<String, AccessPoint_>();

        final List<WifiConfiguration> configs = mWifiManager.getConfiguredNetworks();
        if (configs != null) {
            for (WifiConfiguration config : configs) {
                AccessPoint_ accessPoint = new AccessPoint_(getApplicationContext(), config);
                accessPoint.update(mLastInfo, mLastState);
        ...
        if (results != null) {
            for (ScanResult result : results) {
                ...
                boolean found = false;
                for (AccessPoint_ accessPoint : apMap.getAll(result.SSID)) {
                    if (accessPoint.update(result))
                        found = true;
                }
                if (!found) {
                    AccessPoint_ accessPoint = new AccessPoint_(getApplicationContext(), result);
                    accessPoints.add(accessPoint);
                    apMap.put(accessPoint.ssid, accessPoint);
                }
            }
